...
- **Android Auto**: LeggiMi appears among the car's media apps with your Library; pick a document and it is read aloud, with Previous/Play/Next on the car screen and the steering-wheel buttons. It works with the app closed too: the media service keeps a cache of every document's sentences and reads it by itself with your voice and speed, saving the position; when you open the app it picks up exactly there. Both directions share the position: the car continues where the app stopped and vice versa. The car screen shows a cover drawn by LeggiMi — the comic waveform (a new shape for every sentence), the title and the chapter — since media apps cannot draw anything else there. LeggiMi does not appear among the phone's active players until something is read, and Stop (or swiping the card away) really closes it. Until the app is on Google Play, Android Auto shows it only with *Unknown sources* enabled in its developer settings (phone Settings › Apps › Android Auto › tap the version ten times › ⋮ › Developer settings › Unknown sources).
